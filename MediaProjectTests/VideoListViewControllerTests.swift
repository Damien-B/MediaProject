...
class VideoListViewControllerTests: XCTestCase {
    ...
    var vc : VideoListViewController!
    
    override func setUp() {
        super.setUp()
        
        let storyboard = UIStoryboard(name: "Main", bundle: Bundle(for: type(of: self)))
        vc = storyboard.instantiateViewController(withIdentifier: "VideoListViewController") as! VideoListViewController
    ...
    func testFetchVideo() {
        ...
        self.waitForExpectations(timeout: 5) { error in
            XCTAssertGreaterThan(self.vc.videos.count, 0);
        }
    }
